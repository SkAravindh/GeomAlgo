...
EstablishConnectivity::EstablishConnectivity(Triangle* T, int C_level,std::shared_ptr<Mesh> &Mesh ) {
    this->level = C_level;
...
void EstablishConnectivity::loadParameters(Triangle *TInput, int Refinement_Level) {
    ...
    std::vector<Triangle*> TV;
    pMeshptr->printInfo();
    EstablishConnectivity(TInput,0,pMeshptr);
...
void EstablishConnectivity::divideKE(Triangle *parent, Triangle *t2,std::shared_ptr<Mesh> &Mesh ) {
   ...
   Mesh->CreateTriangle(p0,mid,O_po,t2);
   Mesh->CreateTriangle(mid,p1,O_po,t2);
